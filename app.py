...
if "first_load" not in st.session_state:
    ...
    st.markdown("### 🧠 Random Fact")  # Small heading
    st.markdown(f"<p style='font-size:20px;'>{random.choice(fun_facts)}</p>", unsafe_allow_html=True)
    ...
    for i in range(100):
        ...
        loading_text.text(f"Loading... {i}%")  

    st.snow()
    st.toast("Hey Stranger , Welcome to my awesome app!")


    st.success("🎉 Loaded successfully!")
else:
    # After the first load, show a normal loader
